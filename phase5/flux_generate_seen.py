...
"""
=============================================================================
Phase 5 - FLUX Seen Prompts LoRA Inference (epoch-0)
=============================================================================
Generates images for the same 10 seen prompts using FLUX + LoRA fine-tuned
weights from epoch-0.safetensors.
=============================================================================
"""
...
def main():
    parser = argparse.ArgumentParser(description="FLUX Seen Prompts LoRA Inference")
    parser.add_argument("--epoch", type=int, default=0, help="LoRA epoch to use (default: 0)")
    ...
    possible_lora_paths = [
        Path(f"/home/project_id_10/DiffSynth-Studio/data/id10/flux/models/Flux_lora/epoch-{args.epoch}.safetensors"),
        Path(f"./data/id10/flux/models/Flux_lora/epoch-{args.epoch}.safetensors"),
        Path(f"data/id10/flux/models/Flux_lora/epoch-{args.epoch}.safetensors"),
    ]
    ...
    print(f"[INFO] Running FLUX LoRA inference on {total_prompts} seen prompts (Index {start_idx+1} to {end_idx})")

    # Run Fine-tuned Model (after_seen_{index})
    ...
    print(f"\n[SUCCESS] Completed FLUX LoRA generation on seen prompts. Outputs saved in: {output_dir.resolve()}")

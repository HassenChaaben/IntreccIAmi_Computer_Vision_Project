...
"""
=============================================================================
Phase 5 - FLUX Seen Prompts Base Model Inference (Before LoRA)
=============================================================================
Generates images for the 10 seen prompts using the base FLUX.1-dev model
(without any LoRA weights loaded) for comparison.
=============================================================================
"""
...
def main():
    parser = argparse.ArgumentParser(description="FLUX Seen Prompts Base Model Inference")
    ...
    # Output directory
    output_dir = project_root / "Results_before_after_training" / "flux_exemples_before_lora_training_seen_prompts"
    ...
    print("\n=== Generating Base Model FLUX images ===")
    for idx, prompt in enumerate(SEEN_PROMPTS, start=1):
        ...
        if filepath.exists() and filepath.stat().st_size > 10 * 1024:
            print(f"[{idx}/{len(SEEN_PROMPTS)}] Skipping (exists): {filename}")
            continue

        print(f"[{idx}/{len(SEEN_PROMPTS)}] Generating base: {filename}")
        image = pipe(
            prompt=prompt,
    ...
    print(f"\n[SUCCESS] Completed FLUX generation on seen prompts. Outputs saved in: {output_dir.resolve()}")

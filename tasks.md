...
- [ ] **4.3 Professor Lorenzo's Feedback Tasks**
  - [ ] Compile a comparison of caption token lengths generated by the different models (Z-Image, Flux, SDXL) and provide the precise link
  - [ ] Generate one CSV file for each model (Z-Image, Flux, SDXL) including the image file path and the corresponding generated caption (e.g. `captions_zimage.csv`, `captions_flux.csv`, `captions_sdxl.csv`)
